---
layout: page
title: About Me
description: Who am I?
background: '/img/bg-about.jpg'
---

# Introduction
Hello there, I am Jamie Johnson, a data analyst progressing toward becoming a data scientist. Passionate about uncovering insights from complex datasets, I thrive on statistical modeling, machine learning, and data visualization to solve business challenges and optimize decision-making.  

With a strong analytical mindset, I specialize in handling large-scale data processing, automating workflows, and collaborating cross-functionally to deliver impactful solutions. Living with IRLENS means I have to work harder on tasks that may come easier to others, but it has strengthened my adaptability and problem-solving skills—turning challenges into opportunities for growth.  

Always eager to explore new technologies and enhance predictive analytics, I am committed to driving strategic advancements in data science while continuously refining my expertise.  

# Education

|Level|Course|Outcome|
|:-----:|:------:|:----:|
|HE| T Level in Digital Technology and Solutions | MERIT |
|FE| Level 4 Data Analytics | DISTINCTION |
|FE| Level 6 Data Science | In progress |

# Work Experience

|Company|Time|Role|
|:-----:|:------:|:----:|
|Vizulate| March 2022 - December 2022| Junior Analyst |
|Lloyds Banking Group| September 2023 - Present | Stress Testing Analyst |

# Skills  

### Stress Testing & Risk Analysis  
- Designing and implementing scenarios to assess mortgage book resilience under various economic conditions.  

### Data Analysis & Insights  
- Leveraging advanced statistical tools to interpret complex data and extract actionable insights.  

### Reporting & Communication  
- Creating detailed reports and presentations for senior management, highlighting key findings and recommendations.  

### CRM Optimization  
- Enhancing system efficiency by integrating Zoho One tools and improving data accuracy through form integration.  

### Automation & Process Improvement  
- Streamlining workflows with automated invoicing and email responses to improve efficiency and client communication.  

### Collaboration & Compliance  
- Coordinating with cross-functional teams to ensure effective stress testing and adherence to regulatory requirements.  

### Continuous Improvement  
- Refining methodologies for greater accuracy, reliability, and operational efficiency.  

